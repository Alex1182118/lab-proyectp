            ...
            while (curr != null)
            {
                        ...
                        if (edgePos > -1)//edge is found
                        {
                            ...
                            if (takeRight != null)//case 2a) perform rotation with sibling
                            {
                                ...
                                else//take from left sibling
                                {
                                    ...
                                    if (sibling.Hijos.Count > 0)
                                    {
                                        Nodo edge = sibling.RemoveEdge(sibling.Hijos.Count - 1);//move right most edge
                                        curr.InsertEdge(edge);
                                    }
                                }
                            ...
                            {
                                int? pK = null;
                                if (parent.Hijos.Count >= 2)//case 2b
                                {
                                    if (edgePos == 0)//if n is left most node, take parent's first key
                                    {
                                        pK = parent.Pop(0);
                                    }
                                    else if (edgePos == parent.Hijos.Count)//if n is the right most node take parent's right most key
                                    {
                                    ...
                                    if (pK != null)
                                    {
                                        curr.Push(pK.Value);
                                        Nodo sib = null;
                                        if (edgePos != parent.Hijos.Count)//use right sibling if it is not the rightmost node
                                        {
                                            sib = parent.RemoveEdge(edgePos + 1);
                                        }
                                        else
                                        {
                                            sib = parent.RemoveEdge(parent.Hijos.Count - 1);
                                        }
                ...
                if ((rmPos = curr.HasKey(k)) >= 0)
                {
                    ...
                    else//otherwise, replace it with the next higher key
                    {
                        ...
                        {
                            if (successor.Hijos.Count == 0)//just remove it if it is leaf
                            {
                                Nodo p = successor.Padres;
